...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeCistaAmount"))
        settings.setValue("nAnonymizeCistaAmount", 1000);
    nAnonymizeCistaAmount = settings.value("nAnonymizeCistaAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeCistaAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeCistaAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeCistaAmount:
            return QVariant(nAnonymizeCistaAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeCistaAmount:
            nAnonymizeCistaAmount = value.toInt();
            settings.setValue("nAnonymizeCistaAmount", nAnonymizeCistaAmount);
            emit AnonymizeCistaAmountChanged(nAnonymizeCistaAmount);
            break;
        default:
            break;
        }
