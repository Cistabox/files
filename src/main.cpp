...
const string strMessageMagic = "Cista Signed Message:\n";
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
	...
	if (nHeight < 5001) {
		...
		nSubsidy = STATIC_POS_REWARD * 0.625;
	}
	else if (nHeight < 400001) {
		nSubsidy = STATIC_POS_REWARD * 0.5625;
	}
	else {
		nSubsidy = STATIC_POS_REWARD * 0.50;
	}
    return nSubsidy + nFees;
}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
	...
	int64_t ret;
	if (nHeight < 5001) {
		ret = blockValue * 3 / 8;  //37.5%
	}
	else if (nHeight < 200001) {
		ret = blockValue * 4 / 5;  //80%
	}
	else if (nHeight < 400001) {
		ret = blockValue * 0.7777;  //77.77%
	}
	else {
